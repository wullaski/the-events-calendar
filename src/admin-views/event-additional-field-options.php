				...
				<td>
					<div class="tribe-field-heading">
						<h3>
							<span class="tribe-field-type">
								<?php echo isset( $field['type'] ) ? esc_html( stripslashes( $field['type'] ) ) : esc_html_e( 'Add field', 'the-events-calendar' ) ?>
							</span>
					...
					<div class="tribe-field-content">
						...
						<div class="tribe-field-row tribe-field-type">
							<label><?php esc_html_e( 'Field Type', 'the-events-calendar' ); ?></label>
							<select
							...
							>
								<option value="text" <?php selected( isset( $field['type'] ) && $field['type'] == 'text' ) ?>><?php esc_html_e( 'Text', 'the-events-calendar' ) ?></option>
								<option value="textarea" <?php selected( isset( $field['type'] ) && $field['type'] == 'textarea' ) ?>><?php esc_html_e( 'Text Area', 'the-events-calendar' ) ?></option>
								<option value="url" <?php selected( isset( $field['type'] ) && $field['type'] == 'url' ) ?>><?php esc_html_e( 'URL', 'the-events-calendar' ) ?></option>
								<option value="radio" <?php selected( isset( $field['type'] ) && $field['type'] == 'radio' ) ?>><?php esc_html_e( 'Radio', 'the-events-calendar' ) ?></option>
								<option value="checkbox" <?php selected( isset( $field['type'] ) && $field['type'] == 'checkbox' ) ?>><?php esc_html_e( 'Checkbox', 'the-events-calendar' ) ?></option>
								<option value="dropdown" <?php selected( isset( $field['type'] ) && $field['type'] == 'dropdown' ) ?>><?php esc_html_e( 'Dropdown', 'the-events-calendar' ) ?></option>
							</select>
						</div>

						<div class="tribe-field-row tribe-field-label">
							<label><?php esc_html_e( 'Field Label', 'the-events-calendar' ); ?></label>
							<input
								type="text"
								...
								placeholder="<?php esc_attr_e( 'Enter field label', 'the-events-calendar' ) ?>"
							/>
						</div>
						...
						<div class="tribe-field-row tribe-field-options" style='display: <?php echo ( isset( $field['type'] ) && ( $field['type'] == 'radio' || $field['type'] == 'checkbox' || $field['type'] == 'dropdown' ) ) ? 'block' : 'none' ?>;'>
							<label><?php esc_html_e( 'Options (one per line)', 'the-events-calendar' ); ?></label>
							<textarea
								name="custom-field-options[<?php echo esc_attr( $index ); ?>]"
								data-name-template="custom-field-options"
								placeholder="<?php esc_attr_e( 'One per line', 'the-events-calendar' ); ?>"
...
<script type="text/javascript">
	...
	jQuery( document ).ready( function ( $ ) {
		...
		var textFieldLabel      = '<?php echo esc_js( __( 'Text', 'the-events-calendar' ) ) ?>';
		var textareaFieldLabel  = '<?php echo esc_js( __( 'Text Area', 'the-events-calendar' ) ) ?>';
		var urlFieldLabel       = '<?php echo esc_js( __( 'URL', 'the-events-calendar' ) ) ?>';
		var dropdownFieldLabel  = '<?php echo esc_js( __( 'Dropdown', 'the-events-calendar' ) ) ?>';
		var radioFieldLabel     = '<?php echo esc_js( __( 'Radio', 'the-events-calendar' ) ) ?>';
		var checkboxFieldLabel  = '<?php echo esc_js( __( 'Checkbox', 'the-events-calendar' ) ) ?>';
		...
		if ( fields_tbl.size() > 0 ) {
			...
			fields_tbl.delegate( '.remove-field', 'click', function () {
				...
				if ( confirm( '<?php echo esc_js( __( 'Are you sure you wish to remove this field and its data from all events?', 'the-events-calendar' ) ); ?>' ) ) {
					if ( 'yes' === persisted ) {
